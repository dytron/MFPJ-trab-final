...
class OBB extends Volume {
  ...
  center(p) {
    if (p != undefined) this.points[4] = p;
  ...
  overlaps(v) {
    if (v instanceof OBB) {
      ...
      for (let p of v.points) if (this.hasPoint(p)) return true;

      for (let p of this.points) if (v.hasPoint(p)) return true;

      // Caso 2: Interseção dos segmentos
      for (let line of this.getLines()) {
        for (let line2 of v.getLines()) {
          if (checkCollision(line[0], line[1], line2[0], line2[1])) return true;
        }
      }
    }
    ...
    if (v instanceof Circle) {
      let circle_temp = new Circle();
      circle_temp.center(v.points[0]);
      circle_temp.radius = v.radius;

      circle_temp.center(circle_temp.center().sub(this.center()));

      let angle = -atan2(this.u.y, this.u.x);

      circle_temp.center(circle_temp.center().rot(angle));

      let AABB_temp = new AABB();

      AABB_temp.fit([
        new vec2(this.side2 / 2, this.side1 / 2),
        new vec2(-this.side2 / 2, -this.side1 / 2),
      ]);

      return circle_temp.overlaps(AABB_temp);
    }
    return false;
  }
...
class Circle extends Volume {
  fit(group) {
    let currentCenter = group[0].copy();
    this.center(currentCenter);
    let minRadius2 = Infinity;
    if (group.length === 2) {
      this.center(group[0].add(group[1]).mul(1/2));
      minRadius2 = group[0].sub(group[1]).mul(1/2).dot();
      this.radius = sqrt(minRadius2);
    }
    for (let k = 0; k < 100; k++) {
      currentCenter = this.center().copy()
      for (let i = 0; i < 6; i++) {
...
function checkCollision(A, B, C, D) {
  ...
  if (AB.cross(AC) * AB.cross(AD) > 0) return false;

  let CD = D.sub(C);
  let CA = A.sub(C);
  let CB = B.sub(C);

  if (CD.cross(CA) * CD.cross(CB) > 0) return false;
